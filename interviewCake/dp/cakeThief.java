...
public class Solution {
    ...
    public static long maxDuffelBagValue(CakeType[] cakeTypes, int weightCapacity) {
        ...
        // maximum problem, gonna need to store past values 
        // we can try to go by maximimum shillings first and then downwards,, ratios?
        // array to hold the values for the caketype
        int[] res = new int[weightCapacity+1];
        
        for (int i=0; i < res.length; i++) {
            for (CakeType cake: cakeTypes) {
                int weight = i;
                int value = 0;
                if (cake.weight == 0 && cake.value != 0) throw new RuntimeException();
                if (cake.weight == weight) {
                    value = cake.value;
                } else if (cake.weight < weight)  {
                    value = cake.value + res[weight-cake.weight];
                }
                res[weight] = Math.max(res[weight], value);
            }
        }
